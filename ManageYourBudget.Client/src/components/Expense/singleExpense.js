...
export default class SingleExpense extends React.Component {
    state = {
    ...
    onAddedInternal = async (editedData) => {
        ...
        return (
            <Fragment>
                ...
                <Responsive className={classNameToOfRow} as={Table.Row} minWidth={Responsive.onlyTablet.minWidth}>
                    ...
                    <Table.Cell textAlign="center">{moment(expense.date).format(DATE_FORMAT)}</Table.Cell>
                ...
                <Responsive as={Segment} className={classNameToOfRow} {...Responsive.onlyMobile}>
                    ...
                    {this.renderProp('Date', moment(expense.date).format(DATE_FORMAT))}
